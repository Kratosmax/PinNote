...
> **0.6.0 升级提示：** `0.6.0` 的自动更新会在启动新版更新器之前被临时 ZIP 文件锁阻断，因此无法自动安装 `0.6.1`。请从 [最新 Release](https://github.com/Kratosmax/PinNote/releases/latest) 手动下载并安装一次 `0.7.0`；便签和设置保存在 `%LOCALAPPDATA%\PinNote`，不会因覆盖安装或替换便携包而删除。升级后，后续自动更新恢复正常。

## 0.7.0 本轮更新

- 修复提醒时间时、分、秒下拉框箭头与单位文字重叠，支持精确到秒并保留直接输入。
- 修复提醒强度按钮底部边框被容器裁切的问题，统一四边框、间距和窗口高度。
- 待办分组、层级子待办、单项和批量提醒、逾期强调、拖动排序与独立窗口功能已纳入本版本。
- 普通提醒只闪动便签/待办窗口边框，不再额外弹出提醒窗口。
